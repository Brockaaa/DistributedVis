...
void setPointerAddresses(JVMData jvmData, MPI_Comm renderComm) {
    ...
    void * allToAllColorPointerComp = malloc(windowHeight * windowWidth * numSupersegments * 4 * 4);
    void * allToAllDepthPointerComp = malloc(windowWidth * windowHeight * numSupersegments * 4 * 2);
    ...
    jvmData.env->SetLongField(jvmData.obj, allCC, reinterpret_cast<long>(allToAllColorPointerComp));

    jfieldID allDC = jvmData.env->GetFieldID(jvmData.clazz, "allToAllCompressedDepthPointer", "J");
    jvmData.env->SetLongField(jvmData.obj, allDC, reinterpret_cast<long>(allToAllDepthPointerComp));
...
void distributeVDIsWithMultipleCommunicators(JNIEnv *e, jobject clazzObject, jobject subVDICol, jobject subVDIDepth, jint color, jint divisor, jint rank, jint commSize, jlong colPointer, jlong depthPointer, jlong mpiPointer, jint iteration){
    ...
    MPI_Alltoall(ptrCol, windowHeight * windowWidth * numSupersegments * 4 * 4 / divisor, MPI_BYTE, recvBufCol, windowHeight * windowWidth * numSupersegments * 4 * 4 / divisor, MPI_BYTE, partialCommunicator);
#if VERBOSE
    std::cout<<"Finished color all to all"<<std::endl;
#endif
    MPI_Alltoall(ptrDepth, windowHeight * windowWidth * numSupersegments * 4 * 2 / divisor, MPI_BYTE, recvBufDepth, windowHeight * windowWidth * numSupersegments * 4 * 2 / divisor, MPI_BYTE, partialCommunicator);
#if VERBOSE
    printf("Finished both alltoalls\n");
#endif
